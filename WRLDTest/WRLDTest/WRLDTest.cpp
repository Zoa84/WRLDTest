...
int main()
{
	...
	for (unsigned int i = 0; i < vSFeatures.size(); i++) {
		...
		for (unsigned int j = 0; j < vSFeatures.size(); j++) {
			if (i != j) {
				iDistance += GetEuclidDis(vIFeatureData.at(i), vIFeatureData.at(j));
			}
		}
		...
		if (bDebugText) {
			cout << "Total Distance for " << vSFeatures.at(i) << " is: " << iDistance << endl;
		}
	}
	...
	cout << "The largest distance total is: " << sFinalDistance << " " << iFinalDistance << endl;
